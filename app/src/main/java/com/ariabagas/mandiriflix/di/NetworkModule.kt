...
import com.ariabagas.mandiriflix.BuildConfig
...
object NetworkModule {
    ...
    fun provideInterceptor(@ApplicationContext context: Context): Interceptor =
        Interceptor { chain ->
            val url = chain.request().url.newBuilder()
                ...
                .addQueryParameter("include_image_language", IMAGE_LANGUAGE).build()

            val headerName = "Cache-Control"
            val headerValue = if (isNetworkAvailable(context)) "public, max-age=$CACHE_MAX_AGE"
            else "public, only-if-cached, max-stale=$CACHE_MAX_STALE"

            val request =
                chain.request().newBuilder().url(url).header(headerName, headerValue).build()
    ...
    @Singleton
    @Provides
    fun provideHttpLogInterceptor(): HttpLoggingInterceptor = HttpLoggingInterceptor().setLevel(
    ...
    ): OkHttpClient = OkHttpClient.Builder().cache(Cache(context.cacheDir, CACHE_SIZE))
        .addInterceptor(interceptor).addInterceptor(httpLoggingInterceptor).build()
